...
const { shouldBehaveLikeTRC20 } = require('../TRC20.behavior.js');
...
async function fixture() {
  ...
  const token = await ethers.deployContract('$TRC20BridgeableMock', [name, symbol, bridge]);
  await token.$_mint(accounts[0], initialSupply);

  return { bridge, other, accounts, token };
}

describe('TRC20Bridgeable', function () {
  ...
  describe('TRC20 behavior', function () {
