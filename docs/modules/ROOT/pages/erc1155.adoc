...
TIP: ERC-1155 draws ideas from all of xref:trc20.adoc[TRC-20], xref:trc721.adoc[TRC-721], and https://eips.ethereum.org/EIPS/eip-777[ERC-777]. If you're unfamiliar with those standards, head to their guides before moving on.

[[multi-token-standard]]
== Multi Token Standard

The distinctive feature of ERC-1155 is that it uses a single smart contract to represent multiple tokens at once. This is why its xref:api:token/ERC1155.adoc#IERC1155-balanceOf-address-uint256-[`balanceOf`] function differs from TRC-20's and ERC-777's: it has an additional `id` argument for the identifier of the token that you want to query the balance of.

This is similar to how TRC-721 does things, but in that standard a token `id` has no concept of balance: each token is non-fungible and exists or doesn't. The TRC-721 xref:api:token/TRC721.adoc#ITRC721-balanceOf-address-[`balanceOf`] function refers to _how many different tokens_ an account has, not how many of each. On the other hand, in ERC-1155 accounts have a distinct balance for each token `id`, and non-fungible tokens are implemented by simply minting a single one of them.
...
Also note that, unlike TRC-20, ERC-1155 lacks a `decimals` field, since each token is distinct and cannot be partitioned.
...
This is a good thing! It means that the recipient contract has not registered itself as aware of the ERC-1155 protocol, so transfers to it are disabled to *prevent tokens from being locked forever*. As an example, https://etherscan.io/token/0xa74476443119A942dE498590Fe1f2454d7D4aC0d?a=0xa74476443119A942dE498590Fe1f2454d7D4aC0d[the Golem contract currently holds over 350k `GNT` tokens], and lacks methods to get them out of there. This has happened to virtually every TRC20-backed project, usually due to user error.
