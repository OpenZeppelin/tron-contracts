...
On TRON, the `verify` function performs the verification entirely in Solidity (it is equivalent to `verifySolidity`). Unlike the upstream OpenZeppelin implementation, it does not attempt the https://github.com/ethereum/RIPs/blob/master/RIPS/rip-7212.md[RIP-7212] precompile: the native secp256r1 precompile specified for the TVM by https://github.com/tronprotocol/tips/blob/master/tip-7951.md[TIP-7951] (at address `0x100`) is not assumed to be enabled. This removes any dependency on precompile availability at the cost of higher Energy consumption per verification.
...
The library is a thin wrapper around the opcode. It handles edge cases gracefully by returning zero for the current block, future blocks, or blocks older than 256, matching the VM's native behavior. Note that the extended https://eips.ethereum.org/EIPS/eip-2935[EIP-2935] history storage — specified for the TVM by https://github.com/tronprotocol/tips/blob/master/tip-2935.md[TIP-2935] at the same canonical address — is not yet active on TRON mainnet, so access cannot currently be extended beyond the 256-block window.
