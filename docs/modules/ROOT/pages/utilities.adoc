...
At a high level, signatures are a set of cryptographic algorithms that allow for a _signer_ to prove himself as the owner of a _private key_ used to authorize a piece of information (generally a transaction). Natively, the EVM supports the Elliptic Curve Digital Signature Algorithm (https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm[ECDSA]) using the secp256k1 curve, however other signature algorithms such as P256 and RSA are supported.
...
The data signer can be recovered with xref:api:utils/cryptography.adoc#ECDSA-recover-bytes32-bytes-[`ECDSA.recover`], and its address compared to verify the signature. Native TRON wallets (e.g. TronWeb's `signMessageV2` and TronLink) will hash the data to sign and add the prefix `\x19TRON Signed Message:\n`, so when attempting to recover the signer of a TRON signed message hash, you'll want to use xref:api:utils/cryptography.adoc#MessageHashUtils-toTronSignedMessageHash-bytes32-[`toTronSignedMessageHash`].
...
function _verifyContractSignature(address signer, bytes32 hash, bytes memory signature) internal view returns (bool) {
    return SignatureChecker.isValidTRC1271SignatureNow(signer, hash, signature);
}
----
...
[source,solidity]
----
function _verifyTRC7913Signature(bytes memory signer, bytes32 hash, bytes memory signature) internal view returns (bool) {
    return SignatureChecker.isValidSignatureNow(signer, hash, signature);
}
----
...
* xref:api:utils.adoc#ITRC165[`ITRC165`] — this is the ERC-165 interface that defines xref:api:utils.adoc#ITRC165-supportsInterface-bytes4-[`supportsInterface`]. When implementing ERC-165, you'll conform to this interface.
* xref:api:utils.adoc#TRC165[`TRC165`] — inherit this contract if you'd like to support interface detection using a lookup table in contract storage. You can register interfaces using xref:api:utils.adoc#TRC165-_registerInterface-bytes4-[`_registerInterface(bytes4)`]: check out example usage as part of the TRC-721 implementation.
* xref:api:utils.adoc#TRC165Checker[`TRC165Checker`] — TRC165Checker simplifies the process of checking whether or not a contract supports an interface you care about.
* include with `using TRC165Checker for address;`
* xref:api:utils.adoc#TRC165Checker-_supportsInterface-address-bytes4-[`myAddress._supportsInterface(bytes4)`]
* xref:api:utils.adoc#TRC165Checker-_supportsAllInterfaces-address-bytes4---[`myAddress._supportsAllInterfaces(bytes4[\])`]
...
contract MyContract {
    ...
    bytes4 private InterfaceId_TRC721 = 0x80ac58cd;

    /**
     * @dev transfer a TRC-721 token from this contract to someone else
     */
    function transferTRC721(
        address token,
        address to,
        uint256 tokenId
    )
        public
    {
        require(token.supportsInterface(InterfaceId_TRC721), "IS_NOT_721_TOKEN");
...
This is especially useful for building URL-safe tokenURIs for both xref:api:token/TRC721.adoc#ITRC721Metadata-tokenURI-uint256-[`TRC-721`] or xref:api:token/TRC1155.adoc#ITRC1155MetadataURI-uri-uint256-[`TRC-1155`]. This library provides a clever way to serve URL-safe https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs/[Data URI] compliant strings to serve on-chain data structures.

Here is an example to send JSON Metadata through a Base64 Data URI using a TRC-721:
...
xref:api:utils.adoc#Blockhash[`Blockhash`] provides access to historical block hashes through the native `BLOCKHASH` opcode, which exposes the hashes of the most recent 256 blocks.

The library is a thin wrapper around the opcode. It handles edge cases gracefully by returning zero for the current block, future blocks, or blocks older than 256, matching the VM's native behavior. Note that TVM does not implement https://eips.ethereum.org/EIPS/eip-2935[EIP-2935] history storage, so access cannot be extended beyond the 256-block window.
...
IMPORTANT: Only the 256 most recent block hashes are available. Any query for an older block, the current block, or a future block returns `bytes32(0)`. Treat a zero result as "unavailable" rather than a valid hash.
