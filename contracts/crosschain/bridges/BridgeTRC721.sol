...
// OpenZeppelin Tron Contracts (last updated v5.7.0-rc.0) (crosschain/bridges/BridgeTRC721.sol)
...
/**
 * @dev This is a variant of {BridgeNonFungible} that implements the bridge logic for TRC-721 tokens that do not expose
 * a crosschain mint and burn mechanism. Instead, it takes custody of bridged assets.
 */
// slither-disable-next-line locked-ether
abstract contract BridgeTRC721 is BridgeNonFungible {
    ITRC721 private immutable _token;

    constructor(ITRC721 token_) {
        _token = token_;
    }

    /// @dev Return the address of the TRC721 token this bridge operates on.
    function token() public view virtual returns (ITRC721) {
        return _token;
    }
     ...
     * on that chain. Similarly to the underlying token's {TRC721-transferFrom} function, this function can be called
    ...
    function crosschainTransferFrom(address from, bytes memory to, uint256 tokenId) public virtual returns (bytes32) {
        // Permission is handled using the TRC721's allowance system. This check replicates `TRC721._isAuthorized`.
        address spender = _msgSender();
        require(
            from == spender || token().isApprovedForAll(from, spender) || token().getApproved(tokenId) == spender,
            ITRC721Errors.TRC721InsufficientApproval(spender, tokenId)
        );
