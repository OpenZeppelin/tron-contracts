...
// OpenZeppelin Tron Contracts (last updated v5.6.0) (crosschain/TRC7786Recipient.sol)
 ...
 * @custom:stateless
 */
abstract contract TRC7786Recipient is ITRC7786Recipient {
    /// @dev Error thrown if the gateway is not authorized to send messages to this contract on behalf of the sender.
    error TRC7786RecipientUnauthorizedGateway(address gateway, bytes sender);

    /// @inheritdoc ITRC7786Recipient
    function receiveMessage(
        ...
        if (!_isAuthorizedGateway(msg.sender, sender)) {
            revert TRC7786RecipientUnauthorizedGateway(msg.sender, sender);
        }

        _processMessage(msg.sender, receiveId, sender, payload);

        return ITRC7786Recipient.receiveMessage.selector;
    }
