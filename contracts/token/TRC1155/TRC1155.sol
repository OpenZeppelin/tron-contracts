...
// OpenZeppelin Tron Contracts (last updated v5.6.0) (token/TRC1155/TRC1155.sol)

pragma solidity ^0.8.24;

import {ITRC1155} from "./ITRC1155.sol";
...
abstract contract TRC1155 is Context, TRC165, ITRC1155, ITRC1155MetadataURI, ITRC1155Errors {
     ...
     * https://eips.ethereum.org/EIPS/eip-1155#metadata[defined in the standard].
     *
    ...
    /**
     * @dev See {ITRC1155-balanceOfBatch}.
     *
     * Requirements:
     *
    ...
    function balanceOfBatch(
        ...
        if (accounts.length != ids.length) {
            revert TRC1155InvalidArrayLength(ids.length, accounts.length);
        }
    ...
    /// @dev Checks if `operator` is authorized to transfer tokens owned by `owner`. Reverts with {TRC1155MissingApprovalForAll} if not.
    function _checkAuthorized(address operator, address owner) internal view virtual {
        if (owner != operator && !isApprovedForAll(owner, operator)) {
            revert TRC1155MissingApprovalForAll(operator, owner);
        }
    }
     ...
     * Requirements:
     *
     * - If `to` refers to a smart contract, it must implement either {ITRC1155Receiver-onERC1155Received}
     *   or {ITRC1155Receiver-onERC1155BatchReceived} and return the acceptance magic value.
     * - `ids` and `values` must have the same length.
     *
     * NOTE: The TRC-1155 acceptance check is not performed in this function. See {_updateWithAcceptanceCheck} instead.
     */
    function _update(address from, address to, uint256[] memory ids, uint256[] memory values) internal virtual {
        if (ids.length != values.length) {
            revert TRC1155InvalidArrayLength(ids.length, values.length);
        }
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from != address(0)) {
                ...
                if (fromBalance < value) {
                    revert TRC1155InsufficientBalance(from, fromBalance, value, id);
                }
                unchecked {
     ...
     * {ITRC1155Receiver-onERC1155Received} or {ITRC1155Receiver-onERC1155BatchReceived} on the receiver address if it
     ...
     * - If `to` refers to a smart contract, it must implement {ITRC1155Receiver-onERC1155Received} and return the
     * acceptance magic value.
     */
    function _safeTransferFrom(address from, address to, uint256 id, uint256 value, bytes memory data) internal {
        if (to == address(0)) {
            revert TRC1155InvalidReceiver(address(0));
        }
        if (from == address(0)) {
            revert TRC1155InvalidSender(address(0));
        }
    ...
    /**
     * @dev xref:ROOT:trc1155.adoc#batch-operations[Batched] version of {_safeTransferFrom}.
     *
     * Emits a {TransferBatch} event.
     *
     * Requirements:
     *
     * - If `to` refers to a smart contract, it must implement {ITRC1155Receiver-onERC1155BatchReceived} and return the
     * acceptance magic value.
     * - `ids` and `values` must have the same length.
     */
    ...
    /**
     * @dev xref:ROOT:trc1155.adoc#batch-operations[Batched] version of {_mint}.
     *
     * Emits a {TransferBatch} event.
     *
     * Requirements:
    ...
    /**
     * @dev xref:ROOT:trc1155.adoc#batch-operations[Batched] version of {_burn}.
     *
     * Emits a {TransferBatch} event.
     *
     * Requirements:
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual {
        if (owner == address(0)) {
            revert TRC1155InvalidApprover(address(0));
        }
        if (operator == address(0)) {
            revert TRC1155InvalidOperator(address(0));
        }
