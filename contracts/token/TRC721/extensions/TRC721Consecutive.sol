...
// OpenZeppelin Tron Contracts (last updated v5.5.0) (token/TRC721/extensions/TRC721Consecutive.sol)
...
abstract contract TRC721Consecutive is ITRC2309, TRC721 {
     ...
     * Any batch mint not emitting the {ITRC721-Transfer} event outside of the constructor
     * is non TRC-721 compliant.
     */
    error TRC721ForbiddenBatchMint();

    /**
     * @dev Exceeds the max amount of mints per batch.
     */
    error TRC721ExceededMaxBatchMint(uint256 batchSize, uint256 maxBatch);

    /**
     * @dev Individual minting is not allowed.
     */
    error TRC721ForbiddenMint();
    ...
    /**
     * @dev See {TRC721-_ownerOf}. Override that checks the sequential ownership structure for tokens that have
     ...
     *
     * CAUTION: Does not emit a `Transfer` event. This is TRC-721 compliant as long as it is done inside of the
    ...
    function _mintConsecutive(address to, uint96 batchSize) internal virtual returns (uint96) {
        ...
        if (batchSize > 0) {
            if (address(this).code.length > 0) {
                revert TRC721ForbiddenBatchMint();
            }
            if (to == address(0)) {
            ...
            if (batchSize > maxBatchSize) {
                revert TRC721ExceededMaxBatchMint(batchSize, maxBatchSize);
            }
    ...
    /**
     * @dev See {TRC721-_update}. Override version that restricts normal minting to after construction.
     *
     * WARNING: Using {TRC721Consecutive} prevents minting during construction in favor of {_mintConsecutive}.
