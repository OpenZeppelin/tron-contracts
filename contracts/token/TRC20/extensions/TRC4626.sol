...
// OpenZeppelin Tron Contracts (last updated v5.7.0-rc.0) (token/TRC20/extensions/TRC4626.sol)
 ...
 * https://github.com/tronprotocol/tips/blob/master/tip-4626.md[TIP-4626] (the TRON-side
 * analogue of https://eips.ethereum.org/EIPS/eip-4626[EIP-4626]).
 *
 * This extension allows the minting and burning of "shares" (represented using the TRC-20 inheritance) in exchange for
 * underlying "assets" through standardized {deposit}, {mint}, {redeem} and {burn} workflows. This contract extends
 * the TRC-20 standard. Any additional extensions included along it would affect the "shares" token represented by this
 * contract and not the "assets" token which is an independent contract.
 *
 * [CAUTION]
 * ====
 * In empty (or nearly empty) TRC-4626 vaults, deposits are at high risk of being stolen through frontrunning
 ...
 * underlying math can be found xref:ROOT:trc4626.adoc#inflation-attack[here].
 *
 ...
 *
 * To learn more, check out our xref:ROOT:trc4626.adoc[TRC-4626 guide].
 * ====
 *
 * [NOTE]
 * ====
 ...
 * ====
 */
abstract contract TRC4626 is TRC20, ITRC4626 {
    using Math for uint256;

    ITRC20 private immutable _asset;
    uint8 private immutable _underlyingDecimals;

    /**
     * @dev Attempted to deposit more assets than the max amount for `receiver`.
     */
    error TRC4626ExceededMaxDeposit(address receiver, uint256 assets, uint256 max);

    /**
     * @dev Attempted to mint more shares than the max amount for `receiver`.
     */
    error TRC4626ExceededMaxMint(address receiver, uint256 shares, uint256 max);

    /**
     * @dev Attempted to withdraw more assets than the max amount for `owner`.
     */
    error TRC4626ExceededMaxWithdraw(address owner, uint256 assets, uint256 max);
     ...
     */
    function decimals() public view virtual override(ITRC20Metadata, TRC20) returns (uint8) {
    ...
    function deposit(uint256 assets, address receiver) public virtual returns (uint256) {
        ...
        if (assets > maxAssets) {
            revert TRC4626ExceededMaxDeposit(receiver, assets, maxAssets);
        }
    ...
    function mint(uint256 shares, address receiver) public virtual returns (uint256) {
        ...
        if (shares > maxShares) {
            revert TRC4626ExceededMaxMint(receiver, shares, maxShares);
        }
    ...
    function withdraw(uint256 assets, address receiver, address owner) public virtual returns (uint256) {
        ...
        if (assets > maxAssets) {
            revert TRC4626ExceededMaxWithdraw(owner, assets, maxAssets);
        }
    ...
    function redeem(uint256 shares, address receiver, address owner) public virtual returns (uint256) {
        ...
        if (shares > maxShares) {
            revert TRC4626ExceededMaxRedeem(owner, shares, maxShares);
        }
