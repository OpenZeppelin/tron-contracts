...
// OpenZeppelin Tron Contracts (last updated v5.7.0-rc.0) (token/TRC20/extensions/TRC20TransferAuthorization.sol)
...
/**
 * @dev Variant of {TRC3009} that uses keyed sequential nonces as defined in {NoncesKeyed}.
 *
 ...
 * sharing the same key must be used sequentially. This is unlike {TRC20Permit} which uses a single global
 * sequential nonce.
 */
abstract contract TRC20TransferAuthorization is TRC3009, NoncesKeyed {
    /**
     * @dev See {ITRC3009-authorizationState}.
     *
    ...
    function transferWithAuthorization(
        ...
        require(SignatureChecker.isValidSignatureNow(from, hash, signature), TRC3009InvalidSignature());
    ...
    function cancelAuthorization(address authorizer, bytes32 nonce, bytes memory signature) public virtual {
        ...
        require(SignatureChecker.isValidSignatureNow(authorizer, hash, signature), TRC3009InvalidSignature());
