...
// OpenZeppelin Tron Contracts (last updated v5.5.0) (token/TRC6909/TRC6909.sol)

pragma solidity ^0.8.20;

import {ITRC6909} from "../../interfaces/ITRC6909.sol";
...
contract TRC6909 is Context, TRC165, ITRC6909 {
    ...
    error TRC6909InsufficientBalance(address sender, uint256 balance, uint256 needed, uint256 id);
    error TRC6909InsufficientAllowance(address spender, uint256 allowance, uint256 needed, uint256 id);
    error TRC6909InvalidApprover(address approver);
    error TRC6909InvalidReceiver(address receiver);
    ...
    function _mint(address to, uint256 id, uint256 amount) internal {
        if (to == address(0)) {
            revert TRC6909InvalidReceiver(address(0));
        }
    ...
    function _transfer(address from, address to, uint256 id, uint256 amount) internal {
        if (from == address(0)) {
            revert TRC6909InvalidSender(address(0));
        }
        if (to == address(0)) {
            revert TRC6909InvalidReceiver(address(0));
        }
    ...
    function _update(address from, address to, uint256 id, uint256 amount) internal virtual {
        ...
        if (from != address(0)) {
            ...
            if (fromBalance < amount) {
                revert TRC6909InsufficientBalance(from, fromBalance, amount, id);
            }
            unchecked {
    ...
    function _approve(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        if (owner == address(0)) {
            revert TRC6909InvalidApprover(address(0));
        }
        if (spender == address(0)) {
            revert TRC6909InvalidSpender(address(0));
        }
    ...
    function _spendAllowance(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        ...
        if (currentAllowance < type(uint256).max) {
            if (currentAllowance < amount) {
                revert TRC6909InsufficientAllowance(spender, currentAllowance, amount, id);
            }
            unchecked {
