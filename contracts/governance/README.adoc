...
This modular system of Governor contracts allows the deployment of on-chain voting protocols similar to https://docs.compound.xyz/v2/governance/[Compound's Governor Alpha & Bravo] and beyond, through the ability to easily customize multiple aspects of the protocol.
...
* <<Governor-votingDelay-,`votingDelay()`>>: Delay (in TRC-6372 clock) since the proposal is submitted until voting power is fixed and voting starts. This can be used to enforce a delay after a proposal is published for users to buy tokens, or delegate their votes.
* <<Governor-votingPeriod-,`votingPeriod()`>>: Delay (in TRC-6372 clock) since the proposal starts until voting ends.
* <<Governor-quorum-uint256-,`quorum(uint256 timepoint)`>>: Quorum required for a proposal to be successful. This function includes a `timepoint` argument (see TRC-6372) so the quorum can adapt through time, for example, to follow a token's `totalSupply`.
...
* *Value*, in sun, that should be sent with the transaction. Most of the time this will be 0. TRX can be deposited before-end or passed along when executing the transaction.
