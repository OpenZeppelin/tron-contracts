...
// OpenZeppelin Tron Contracts (last updated v5.7.0-rc.0) (utils/SimulateCall.sol)
...
library SimulateCall {
    ...
    function getSimulator() internal returns (address instance) {
        ...
        assembly ("memory-safe") {
            ...
            // compute create2 address — TIP-26 prefix is 0x41, not the
            // EVM 0xff. The simulator is materialised by the create2 below at
            // the 0x41-prefixed address; the predictor here must use the same
            // prefix so extcodesize(instance) reads the actual deployment and
            // we don't re-deploy on every call into a slot with no code.
