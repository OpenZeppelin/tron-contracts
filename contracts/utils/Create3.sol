...
// OpenZeppelin Tron Contracts (last updated v5.7.0) (utils/Create3.sol)
 ...
 *
 * IMPORTANT: This library DOES NOT produce reproducible addresses on the TVM and must not be used for
 * counterfactual (deterministic) deployment on TRON. `CREATE3` relies on the intermediate proxy deploying the
 * final contract with a plain `CREATE`, whose address on the EVM is a pure function of `(sender, nonce)`. On the
 * TVM an opcode-level `CREATE` instead derives the address from the transaction hash:
 * `0x41 ++ keccak256(txId ++ nonce)[12:]`, where `txId` is the SHA-256 of the transaction's `raw_data` and
 * `nonce` is java-tron's internal-transaction counter (the proxy address is not even an input). See
 * https://github.com/tronprotocol/tips/blob/master/tip-26.md[TIP-26]. Because that address depends on the runtime
 * transaction rather than on `(sender, nonce)`, {computeAddress} and the address returned by {deploy} DO NOT match
 * where the contract is actually deployed on the TVM, and the same `salt` yields a different address in every
 * transaction. Unlike `CREATE2`, there is no constant that fixes this. Use {Create2} or {Clones}, whose addresses
 * are a pure function of `(deployer, salt, initcode)` and stay predictable on the TVM.
 */
library Create3 {
    /**
     * ===================================[ PROXY CODE ]===================================
     ...
     *
     * WARNING: On the TVM this does not match the actual deployment address. See the library-level note.
     */
