 ...
 * {TRC165}, {TRC165Checker}: Utilities for inspecting interfaces supported by contracts.
 ...
 * {Blockhash}: A library for accessing historical block hashes within the native `BLOCKHASH` opcode's 256-block window.
 ...
 * {Create3}: Variant of {Create2} that deploys at an address independent of the deployed bytecode. NOTE: not usable on the TVM, where the addresses are not reproducible; see the contract.
 * {InteroperableAddress}: Library for formatting and parsing TRC-7930 interoperable addresses.
 ...
 * {SlotDerivation}: Methods for deriving storage slot from https://github.com/tronprotocol/tips/blob/master/tip-7201.md[TIP-7201] (the TRON-side analogue of https://eips.ethereum.org/EIPS/eip-7201[ERC-7201]) namespaces as well as from constructions such as mapping and arrays.
...
Ethereum contracts have no native concept of an interface, so applications must usually simply trust that they are not making an incorrect call. For trusted setups this is a non-issue, but often unknown and untrusted third-party addresses need to be interacted with. There may not even be any direct calls to them! (e.g. TRC-20 tokens may be sent to a contract that lacks a way to transfer them out of it, locking them forever). In these cases, a contract _declaring_ its interface can be very helpful in preventing errors.
