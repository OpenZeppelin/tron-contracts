 ...
 */
abstract contract TRC20VotesLegacyMock is IVotes, TRC20Permit {
    struct Checkpoint {
        uint32 fromBlock;
        uint224 votes;
    }
    ...
    function getPastVotes(address account, uint256 blockNumber) public view virtual returns (uint256) {
        require(blockNumber < block.number, "TRC20Votes: block not yet mined");
    ...
    function delegateBySig(
        ...
        require(nonce == _useNonce(signer), "TRC20Votes: invalid nonce");
    ...
    function _update(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == address(0)) {
            require(totalSupply() <= _maxSupply(), "TRC20Votes: total supply risks overflowing votes");
